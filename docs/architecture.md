...
- CaseFile: 巨鲸建仓、风险响应、交易员复核等样例调查档案
- Transaction: 链上交易
- Asset: Token / NFT / Stablecoin
- TokenProfile: 资产档案、板块标签、风险窗口与相关信号
- AlphaScorecard: Alpha、商业模式、代币价值捕获和风险评分
- LiveAlphaAlert: 由价格、TVL、DEX 热点、钱包标签、解锁和风险事件生成的实时行动队列
- DataConnector: CoinGecko、DeFiLlama、DEX Screener、Binance、Alternative.me、链上浏览器和商业 API 接入配置
- WatchRule: 监控规则
- Alert: 预警事件
...
- `/tokens` 资产雷达
- `/alpha` 市场 Alpha 情报局
- `/alpha/live.html` Live Alpha 雷达
- `/alpha/alerts.html` Live Alpha 预警台
- `/alpha/report.html` Alpha 项目评分报告
- `/alpha/watchlist.html` Alpha 项目观察池
- `/alpha/updates.html` Alpha 评分更新流
- `/entities` 实体标签库
- `/sources` 数据源与实时接入中心
- `/playbooks` 调查手册
- `/casefiles` 案例档案

## 6. Public Data Streams

当前 GitHub Pages 前端可以公开接入的轻量数据源：

- CoinGecko: 主流资产价格、24 小时涨跌和市场背景
- DeFiLlama Protocols: 协议 TVL 和 DeFi 资金沉淀
- DeFiLlama Stablecoins: 稳定币供给、价格和流动性背景
- DeFiLlama Chains: 链级 TVL 分布和生态优先级
- DeFiLlama Security: 公开安全事件样本
- DEX Screener: DEX Boost 热点和早期 Token 注意力
- Binance Market Data: 24 小时成交量、成交笔数和流动性锚点
- Alternative.me: Crypto Fear & Greed 市场情绪

应进入后端代理、缓存或付费数据层的数据源：

- Etherscan、Solscan、Covalent、Moralis: 钱包交易、余额、合约和地址活动
- Dune、The Graph、Flipside: 自定义链上分析和聚合查询
- Nansen、Arkham、TRM、Chainalysis: 地址标签、实体归因和风险标签
- TokenUnlocks、项目治理与公告源: 解锁日历、事件日历和治理变化
- DeFiLlama 大型收益池数据: 端点体积较大，应通过后端缓存后再进入前端

## 7. Daily Automation

当前静态站的每日信息更新采用 GitHub Actions 定时任务：

- Workflow: `.github/workflows/daily-intelligence.yml`
- Schedule: 每天 00:30 UTC，即北京时间 08:30
- Script: `docs/update-daily-intelligence.mjs`
- Output: `assets/daily-intelligence.json`
- Frontend: 首页 `#today` 和 `/intelligence/` 优先读取 JSON，失败时显示静态兜底内容

生成器每天抓取公开轻量数据源，压缩成 5 条核心情报：

- 市场情绪: Alternative.me Fear & Greed + 主流资产波动
- 流动性: Binance 24 小时成交额和成交笔数
- 稳定币: DeFiLlama Stablecoins 供给和价格
- DeFi 与链上资金: DeFiLlama Protocols + Chains
- 风险与热点: DeFiLlama Security + DEX Screener Boost

下一阶段应迁移到后端任务系统：

- 保存每日历史版本，而不是只覆盖最新 JSON
- 增加人工审核状态和发布按钮
- 对接用户观察池，把每日情报自动路由到关注资产、钱包和交易员
- 支持邮件、Telegram、Discord、Webhook 和站内通知
